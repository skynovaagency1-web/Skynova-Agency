...
/* BikesBooking is wired through bikeRentalLink() in lib/affiliate.ts, which
 * every CTA on this page already calls -- so the fix was one function body,
 * not a change here.
 *
 * Their WIDGET (campaign_id=57, promo_id=5472) was built and measured on
 * 17 Sep 2026 and then pulled: its own copy rendered "the perfect mode of
 * transportation on undefined", it illustrated a generic "transportation
 * rentals" banner with a CAR on the bike page, and it fetched
 * http://avsplow.com/a/j over plain HTTP from an HTTPS page, which every
 * browser blocks as mixed content. The link does the job without any of
 * that. If a widget is revisited: it only mounts with async=false on the
 * script tag, because it locates itself via document.currentScript --
 * AffiliateWidget takes that as a prop.
 */
